...
PHP_METHOD(Phalcon_Http_Response, redirect)
{
	...
	if (externalRedirect) {
		...
		if (_0$$5) {
			...
			ZVAL_STRING(&_1$$5, "://");
			ZEPHIR_CALL_FUNCTION(&_2$$5, "strstr", NULL, 386, location, &_1$$5);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, send)
{
	...
	if (Z_TYPE_P(&content) != IS_NULL) {
		...
		if (_3$$5) {
			ZEPHIR_CALL_FUNCTION(NULL, "readfile", NULL, 387, &file);
			zephir_check_call_status();
		}
	}
	if (1) {
...
PHP_METHOD(Phalcon_Http_Response, setFileToSend)
{
	...
	if (Z_TYPE_P(attachmentName) != IS_STRING) {
		ZEPHIR_CALL_METHOD(&basePath, this_ptr, "getbasename", NULL, 388, &filePath_zv);
		zephir_check_call_status();
	} else {
	...
	if (zephir_is_true(attachment)) {
		if ((zephir_function_exists_ex(ZEND_STRL("mb_detect_encoding")) == SUCCESS)) {
			ZEPHIR_CALL_FUNCTION(&_0$$6, "mb_detect_order", NULL, 389);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(&basePathEncoding, "mb_detect_encoding", NULL, 350, &basePath, &_0$$6, &__$true);
			zephir_check_call_status();
		}
		ZEPHIR_INIT_VAR(&_1$$5);
		...
		if (!ZEPHIR_IS_STRING(&basePathEncoding, "ASCII")) {
			ZEPHIR_CALL_FUNCTION(&_2$$7, "rawurlencode", NULL, 354, &basePath);
			zephir_check_call_status();
			...
			ZEPHIR_CALL_FUNCTION(&_6$$8, "addcslashes", NULL, 220, &basePath, &_5$$8);
			zephir_check_call_status();
	...
	if (Z_TYPE_P(&currentHeadersRaw) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&currentHeadersRaw), _4, _5, _2)
		{
			...
			if (_6$$3) {
				...
				ZEPHIR_CALL_FUNCTION(&_8$$3, "strstr", &_9, 386, &key, &_7$$3);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$5) {
					...
					ZEPHIR_CALL_FUNCTION(&_13$$5, "strstr", &_9, 386, &key, &_12$$5);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Http_Response, getBasename)
{
	...
	ZEPHIR_INIT_VAR(&_3);
	ZVAL_STRING(&_3, "@");
	ZEPHIR_CALL_FUNCTION(&_4, "preg_quote", NULL, 382, &_2, &_3);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_5);
	...
	if (zephir_is_true(suffix)) {
		...
		ZVAL_STRING(&_6$$3, "@");
		ZEPHIR_CALL_FUNCTION(&_7$$3, "preg_quote", NULL, 382, suffix, &_6$$3);
		zephir_check_call_status();
