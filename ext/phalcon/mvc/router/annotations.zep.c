...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, handle)
{
	...
	zend_string *uri = NULL, *_20$$18, *_41$$41;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(&handlers) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&handlers), _2)
		{
			...
			if (!(ZEPHIR_IS_EMPTY(&prefix))) {
				...
				ZEPHIR_CALL_METHOD(NULL, &route, "__construct", &_4, 104, &prefix);
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(&_5$$6, &route, "getcompiledpattern", &_6, 493);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (!(ZEPHIR_IS_EMPTY(&prefix))) {
					...
					ZEPHIR_CALL_METHOD(NULL, &route, "__construct", &_4, 104, &prefix);
					zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_28$$29, &route, "getcompiledpattern", &_6, 493);
					zephir_check_call_status();
					...
					if (zephir_memnstr_str(&compiledPattern, SL("^"), "phalcon/Mvc/Router/Annotations.zep", 146)) {
						...
						zephir_preg_match(&_32$$30, &compiledPattern, &uri_zv, &_31$$30, 0, 0 , 0 );
...
PHP_METHOD(Phalcon_Mvc_Router_Annotations, processActionAnnotation)
{
	...
	if (Z_TYPE_P(&_2) != IS_NULL) {
		...
		ZEPHIR_CALL_FUNCTION(&_4$$6, "call_user_func", NULL, 190, &_3$$6, &proxyActionName);
		zephir_check_call_status();
