...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, convertEncoding)
{
	...
	_0 = ZEPHIR_IS_STRING_IDENTICAL(&from_zv, "latin1");
	if (!(_0)) {
		_0 = ZEPHIR_IS_STRING_IDENTICAL(&to_zv, "utf8");
	}
	if (_0) {
		ZEPHIR_RETURN_CALL_FUNCTION("utf8_encode", NULL, 502, &text_zv);
		zephir_check_call_status();
		RETURN_MM();
	}
	_1 = ZEPHIR_IS_STRING_IDENTICAL(&to_zv, "latin1");
	if (!(_1)) {
		_1 = ZEPHIR_IS_STRING_IDENTICAL(&from_zv, "utf8");
	}
	if (_1) {
		ZEPHIR_RETURN_CALL_FUNCTION("utf8_decode", NULL, 288, &text_zv);
		zephir_check_call_status();
		RETURN_MM();
	}
	if ((zephir_function_exists_ex(ZEND_STRL("mb_convert_encoding")) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("mb_convert_encoding", NULL, 329, &text_zv, &from_zv, &to_zv);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, isIncluded)
{
	...
	if (Z_TYPE_P(haystack) == IS_STRING) {
		if ((zephir_function_exists_ex(ZEND_STRL("mb_strpos")) == SUCCESS)) {
			ZEPHIR_CALL_FUNCTION(&_0$$6, "mb_strpos", NULL, 88, haystack, needle);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, slice)
{
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("array_slice", NULL, 508, value, &_5$$9, &length);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, sort)
{
	...
	ZEPHIR_CALL_FUNCTION(NULL, "asort", NULL, 509, &value);
	ZEPHIR_UNREF(&value);
	zephir_check_call_status();
	RETURN_CTOR(&value);
}
