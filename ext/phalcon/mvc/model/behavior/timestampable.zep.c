...
PHP_METHOD(Phalcon_Mvc_Model_Behavior_Timestampable, notify)
{
	...
	zval type_zv, *model, model_sub, options, timestamp, singleField, field, _0, *_1$$6, _2$$6;
	...
	ZEPHIR_CALL_METHOD(&timestamp, this_ptr, "gettimestamp", NULL, 463, &options);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_Model_Behavior_Timestampable, getTimestamp)
{
	...
	if (zephir_array_isset_string_fetch(&format, &options, SL("format"), 0)) {
		ZEPHIR_RETURN_CALL_FUNCTION("date", NULL, 464, &format);
		zephir_check_call_status();
		RETURN_MM();
	}
