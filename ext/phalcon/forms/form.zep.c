...
PHP_METHOD(Phalcon_Forms_Form, isValid)
{
	...
	if (_7) {
		...
		ZEPHIR_CALL_METHOD(NULL, &validation, "__construct", NULL, 341);
		zephir_check_call_status();
	}
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&_8), _9)
		{
			...
			if (Z_TYPE_P(&validators) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&validators), _12$$10)
				{
					...
					ZEPHIR_CALL_METHOD(NULL, &validation, "add", &_14, 342, &name, &validator);
					zephir_check_call_status();
				} ZEND_HASH_FOREACH_END();
			} else {
			...
			if (Z_TYPE_P(&filters) == IS_ARRAY) {
				ZEPHIR_CALL_METHOD(NULL, &validation, "setfilters", &_15, 343, &name, &filters);
				zephir_check_call_status();
			}
		} ZEND_HASH_FOREACH_END();
	} else {
	...
	ZEPHIR_INIT_NVAR(&element);
	ZEPHIR_CALL_METHOD(NULL, &validation, "validate", NULL, 344, data, entity);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&messages, &validation, "getmessages", NULL, 345);
	zephir_check_call_status();
