...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, __construct)
{
	...
	zval callbacks;
	zval filePath_zv, *callbacks_param = NULL, yamlConfig, _0, _1, _2$$5, _3$$5, _4$$7, _5$$7, _6$$7;
	...
	if (ZEPHIR_IS_EMPTY(&callbacks)) {
		...
		ZEPHIR_CALL_METHOD(&yamlConfig, this_ptr, "phpyamlparsefile", NULL, 0, &filePath_zv, &_2$$5, &_3$$5, &callbacks);
		zephir_check_call_status();
	}
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&yamlConfig))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_4$$7, "__construct", NULL, 35, &_6$$7);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, phpYamlParseFile)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("yaml_parse_file", NULL, 202, filename, pos, ndocs, callbacks);
	ZEPHIR_UNREF(ndocs);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Config_Adapter_Yaml, phpExtensionLoaded)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("extension_loaded", NULL, 203, &name_zv);
	zephir_check_call_status();
	RETURN_MM();
}
