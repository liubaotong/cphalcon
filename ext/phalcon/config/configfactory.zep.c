...
PHP_METHOD(Phalcon_Config_ConfigFactory, load)
{
	...
	ZVAL_LONG(&_1, 4);
	ZEPHIR_CALL_FUNCTION(&_2, "pathinfo", NULL, 99, &filePath, &_1);
	zephir_check_call_status();
	if (1 == ZEPHIR_IS_EMPTY(&_2)) {
		ZEPHIR_CALL_FUNCTION(&_3$$3, "lcfirst", NULL, 85, &adapter);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Config_ConfigFactory, newInstance)
{
	...
	zephir_method_globals *ZEPHIR_METHOD_GLOBALS_PTR = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval name_zv, fileName_zv, *params = NULL, params_sub, __$null, definition;
	...
	do {
		if (ZEPHIR_IS_STRING(&name_zv, "grouped") || ZEPHIR_IS_STRING(&name_zv, "ini") || ZEPHIR_IS_STRING(&name_zv, "yaml")) {
...
PHP_METHOD(Phalcon_Config_ConfigFactory, parseConfig)
{
	...
	if (Z_TYPE_P(config) == IS_STRING) {
		...
		ZVAL_LONG(&_0$$3, 4);
		ZEPHIR_CALL_FUNCTION(&extension, "pathinfo", NULL, 99, config, &_0$$3);
		zephir_check_call_status();
	...
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "checkconfigarray", NULL, 204, config);
	zephir_check_call_status();
