...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_Ansi, pad)
{
	...
	ZEPHIR_CALL_FUNCTION(&_3, "chr", NULL, 248, &_0);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_Ansi, unpad)
{
	...
	zval input_zv, *blockSize_param = NULL, last, length, ord, padding, paddingSize, repeat, _0, _1, _2$$4, _3$$4, _4$$3, _5$$3;
	...
	ZEPHIR_CALL_FUNCTION(&ord, "ord", NULL, 249, &last);
	zephir_check_call_status();
	if (ZEPHIR_LE_LONG(&ord, blockSize)) {
		...
		if (ZEPHIR_GT_LONG(&paddingSize, 1)) {
			ZVAL_LONG(&_2$$4, 0);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "chr", NULL, 248, &_2$$4);
			zephir_check_call_status();
