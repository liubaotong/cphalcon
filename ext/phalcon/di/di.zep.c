...
PHP_METHOD(Phalcon_Di_Di, __call)
{
	...
	if (zephir_start_with_str(&method_zv, SL("get"))) {
		...
		ZEPHIR_CALL_FUNCTION(&possibleService, "lcfirst", NULL, 85, &_1$$3);
	...
	if (zephir_start_with_str(&method_zv, SL("set"))) {
		...
		if (zephir_array_isset_long_fetch(&definition, &arguments, 0, 0)) {
			...
			ZEPHIR_CALL_FUNCTION(&_5$$6, "lcfirst", NULL, 85, &_4$$6);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Di, attempt)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 86, definition, &_2);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Di, get)
{
	...
	ZVAL_NULL(&service);
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "resolvealias", NULL, 87, &name);
	zephir_check_call_status();
		...
		if (Z_TYPE_P(&service) != IS_NULL) {
			...
			if (EG(exception)) {
				...
				if (zephir_instance_of_ev(&_10$$7, phalcon_di_exception_serviceresolutionexception_ce)) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_12$$9, "__construct", NULL, 35, &_13$$9);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Di, loadFromPhp)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &services, "__construct", NULL, 88, &filePath_zv);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Di, loadFromYaml)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &services, "__construct", NULL, 89, &filePath_zv, &callbacks);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "loadfromconfig", NULL, 0, &services);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Phalcon_Di_Di, remove)
{
	...
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "resolvealias", NULL, 87, &name);
	zephir_check_call_status();
	...
	if (Z_TYPE_P(&localAliases) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&localAliases), _3)
		{
			...
			if (_5$$5) {
				...
				ZEPHIR_CALL_METHOD(NULL, &_7$$7, "__construct", &_9, 35, &_8$$7);
				zephir_check_call_status();
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_10$$8) {
					...
					ZEPHIR_CALL_METHOD(NULL, &_12$$10, "__construct", &_9, 35, &_13$$10);
					zephir_check_call_status();
...
PHP_METHOD(Phalcon_Di_Di, resolveAlias)
{
	...
	while (1) {
		...
		if (zephir_array_isset(&seen, &current)) {
			...
			ZEPHIR_CALL_FUNCTION(&_3$$4, "sprintf", &_4, 76, &_2$$4, &name_zv);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(NULL, &_1$$4, "__construct", &_5, 35, &_3$$4);
			zephir_check_call_status();
