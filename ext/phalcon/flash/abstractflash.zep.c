...
PHP_METHOD(Phalcon_Flash_AbstractFlash, outputMessage)
{
	...
	if (Z_TYPE_P(message) == IS_ARRAY) {
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(message), _2)
		{
			...
			ZVAL_COPY(&item, _2);
			ZEPHIR_CALL_METHOD(&prepared, this_ptr, "prepareescapedmessage", &_4, 92, &item);
			zephir_check_call_status();
			ZEPHIR_CALL_METHOD(&html, this_ptr, "preparehtmlmessage", &_5, 93, &type_zv, &prepared);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Flash_AbstractFlash, prepareHtmlMessage)
{
	...
	ZEPHIR_CALL_METHOD(&cssClasses, this_ptr, "checkclasses", NULL, 94, &_1, &type_zv);
	zephir_check_call_status();
	zephir_read_property(&_2, this_ptr, ZEND_STRL("cssIconClasses"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&cssIconClasses, this_ptr, "checkclasses", NULL, 94, &_2, &type_zv);
	zephir_check_call_status();
	zephir_read_property(&_3, this_ptr, ZEND_STRL("interpolator"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "gettemplate", NULL, 95, &cssClasses, &cssIconClasses);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_5);
