...
PHP_METHOD(Phalcon_Filter_Sanitize_Ip, __invoke)
{
	...
	if (zephir_memnstr_str(&ipInput, SL("/"), "phalcon/Filter/Sanitize/Ip.zep", 32)) {
		...
		ZEPHIR_CALL_FUNCTION(&filtered, "filter_var", NULL, 283, &ip, &_0$$3, &_1$$3);
		zephir_check_call_status();
		if (ZEPHIR_IS_FALSE_IDENTICAL(&filtered)) {
			RETURN_MM_BOOL(0);
		}
		ZEPHIR_CALL_FUNCTION(&_2$$3, "ctype_digit", NULL, 285, &mask);
		zephir_check_call_status();
	...
	ZVAL_LONG(&_5, 275);
	ZVAL_LONG(&_6, filter);
	ZEPHIR_RETURN_CALL_FUNCTION("filter_var", NULL, 283, &ipInput, &_5, &_6);
	zephir_check_call_status();
	RETURN_MM();
}
