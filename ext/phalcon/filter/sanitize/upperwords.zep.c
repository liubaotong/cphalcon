...
PHP_METHOD(Phalcon_Filter_Sanitize_UpperWords, __invoke)
{
	...
	if (1 == (zephir_function_exists_ex(ZEND_STRL("mb_convert_case")) == SUCCESS)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("mb_convert_case", NULL, 12, &input_zv, &_0$$3, &_1$$3);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_CALL_FUNCTION(&_2, "utf8_decode", NULL, 289, &input_zv);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("ucwords", NULL, 292, &_2);
	zephir_check_call_status();
	RETURN_MM();
}
